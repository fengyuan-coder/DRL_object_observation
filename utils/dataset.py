...
def read_voc_dataset(download=True, year='2007'):
    """
        根据输入年份从Pascal VOC数据集中检索验证和训练数据记录器的函数
        指定图片尺寸，返回每个年份的训练数据(train)和测试数据(val)
    """
    ...
    # 提取训练数据(train); .\datasets\VOCdevkit\VOC2007\ImageSets\Main\train.txt
    voc_data =  torchvision.datasets.VOCDetection(PATH, year=year, image_set='train', download=download, transform=T)   # 类型<class 'torchvision.datasets.voc.VOCDetection'>
    train_loader = DataLoader(voc_data,shuffle=True)

    # 提取训练数据(val); .\datasets\VOCdevkit\VOC2007\ImageSets\Main\val.txt
    voc_val =  torchvision.datasets.VOCDetection(PATH, year=year, image_set='val',download=download, transform=T)   # 类型<class 'torchvision.datasets.voc.VOCDetection'>
